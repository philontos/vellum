...
def workspace(limit: int = 50, before: int | None = None) -> dict:
    ...
    return {
        ...
        "prompt_versions": obs.list_conversation_prompt_versions(),
...
def round_detail(assistant_turn: int) -> dict:
    ...
    return {
        ...
        "runs": obs.conversation_runs_for_turn(assistant_turn),
    }
...
def create_prompt_version(name: str, content: str) -> dict:
    ...
    return obs.create_conversation_prompt_version(name, content)


def start_run(prepared: PreparedReplay) -> dict:
    model_name = resolve_structured_llm_config().get("model") or None
    run_id = obs.create_conversation_run({
    ...
    return obs.get_conversation_run(run_id)
...
async def run_events(run_id: int, prepared: PreparedReplay):
    ...
    try:
        ...
        yield {"type": "done", "run": obs.get_conversation_run(run_id)}
    except asyncio.CancelledError:
        obs.fail_conversation_run(run_id, "Replay cancelled")
        raise
    except Exception as exc:
        message = f"{type(exc).__name__}: {exc}"
        obs.fail_conversation_run(run_id, message)
        yield {"type": "done", "run": obs.get_conversation_run(run_id)}
