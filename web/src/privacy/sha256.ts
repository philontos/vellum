// Portable SHA-256 for browsers that hide SubtleCrypto on plain HTTP origins.
// This keeps the local privacy-PIN format stable; the PIN is explicitly not an
// authentication or data-security boundary (see pin.ts).
...
export function sha256(input: Uint8Array): Uint8Array {
  ...
  for (let offset = 0; offset < paddedLength; offset += 64) {
    ...
    for (let i = 0; i < 64; i++) {
      ...
      const temp1 = (h + sum1 + choose + ROUND_CONSTANTS[i] + words[i]) >>> 0;
